...
original_author="consokodev"
original_name="tracking_token_social"
original_urlname="tracking-token-social"
original_description="Awesome tracking_token_social created by consokodev" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/tracking_token_social "${name}"
...
then
    ...
    echo "then run 'tracking_token_social' or 'python -m tracking_token_social'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'tracking_token_social' or 'python -m tracking_token_social' or 'poetry run tracking_token_social'"
fi
