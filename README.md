...
- 🎯 Entry points to execute your program using `python -m <tracking_token_social>` or `$ tracking_token_social` with basic CLI argument parsing.
...
---
# tracking_token_social Flask Application

Awesome tracking_token_social created by consokodev
...
```bash
git clone https://github.com/consokodev/tracking-token-social tracking_token_social
cd tracking_token_social
make install
```
...
```bash
pip install tracking_token_social
```
...
```bash
$ tracking_token_social
```

or

```bash
$ python -m tracking_token_social
```
...
```bash
tracking_token_social create-db   # run once
tracking_token_social populate-db  # run once (optional)
tracking_token_social add-user -u admin -p 1234  # ads a user
tracking_token_social run
```
